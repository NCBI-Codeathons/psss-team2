...
def main(args):
    ...
    if len(extras):
        print(
            (
                f"Found {len(extras)} extra contigs in {args.pred_tsv}. "
                "Discarding before computing metrics."
            ),
            file=sys.stderr,
        )

        mask = np.logical_or(pred_df["qseqid"].isin(ctgs), pred_df["sseqid"].isin(ctgs))
    ...
    for i in range(n_bins):
        ...
        bin_metrics = calc_scores(
            true_g[row_mask, col_mask] >= s, pred_g[row_mask, col_mask]
        )
        for m in metrics:
...
def parse_argparse_args():
    ...
    parser.add_argument(
        "true_tsv", type=str, help="true containments in tabular BLAST output"
    )
    parser.add_argument(
        "pred_tsv", type=str, help="predicted containments in tabular BLAST output"
    )
    parser.add_argument(
        "-o", "--output", type=str, help="the file to save results to", default=None
    )
