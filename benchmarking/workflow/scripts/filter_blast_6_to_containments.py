...
def find_containments(
    ...
    reference_index = pd.read_csv(
        reference_index_path, sep="\t", header=None, names=FAI_HEADER
    )
    ...
    if has_header:
        ...
        search_results = pd.read_csv(
            results_path, sep="\t", header=None, names=BOUTFMT6_COLUMNS
        )

    is_containment = search_results.apply(
        check_containment,
        axis=1,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--results_path",
        type=str,
        help="BLAST format 6 table of alignments to check for containments.",
    )
    parser.add_argument(
        "-q",
        "--query_index_path",
        type=str,
        help="Index of query sequences in samtools faidx format.",
    )
    parser.add_argument(
        "-r",
        "--reference_index_path",
        type=str,
        help="Index of reference sequences in samtools faidx format.",
    )
    parser.add_argument(
        "-o",
        "--output_path",
        type=str,
        help="Path to write containments in BLAST format 6.",
    )
    parser.add_argument(
        "--percent_identity",
        type=float,
    ...
    find_containments(
        args.results_path,
        args.query_index_path,
        args.reference_index_path,
        args.output_path,
